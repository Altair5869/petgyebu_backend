...
| n84 아이템 장착/해제 | action | s7 | Q8: 구매한 꾸미기 아이템을 파트너에게 장착, 결과가 메인 홈에 반영 |
...
n58 보상 보관함 화면
  → n59 보유 아이템 목록
  → n84 아이템 장착/해제
  → n29 메인 홈 (장착 결과가 캐릭터 표시에 반영)
```

n84는 Q8 결정으로 추가됐다. 장착한 아이템은 캐릭터 상태 6단계 애니메이션 위에 겹쳐 그리며, 상태가 바뀌어도 장착 상태는 유지된다. 이 노드가 없으면 구매(n77)에서 보상 루프가 끊겨 크레딧을 모을 이유가 사라진다.
