# 유저플로우: 반려동물 감정 기반 소비 관리 가계부 앱

- 원본: Manyfast 유저플로우 "새 플로우 1" (v1, versionId: `eb4a9d20-55bd-4492-bb7a-11bead78bc0d`)
- 2026-08-29 업데이트: `02-requirements-features.md` 통합 반영 결과를 기준으로 노드 12개·엣지 16개 추가
- 2026-09-15 업데이트: MVP 재점검 Q1~Q14 결정(소셜 로그인·약관 동의·금융 동의·회원 탈퇴·아이템 장착)을 반영해 노드 6개 추가, 엣지 4개 제거·13개 추가
- 노드 84개, 엣지 112개, 섹션 9개
- **디바이스 범위**: Q14 결정에 따라 모든 화면은 Flutter 모바일 앱 기준이다. 웹은 랜딩 페이지만 두고 이 플로우에 포함하지 않는다.

## 노드 타입 범례

| Mermaid 도형 | 타입 | 의미 |
|---|---|---|
| `([Label])` | start | 진입점 |
| `[Label]` | page | 화면 |
| `{Label}` | branch | 분기 조건 |
| `(Label)` | action | 사용자/시스템 동작 |
| `[(Label)]` | data | 데이터 조회/표시 |

## 섹션 목록

| 섹션 ID | 이름 | 관련 요구사항 |
|---|---|---|
| s1 | 인증 및 최초 설정 | R-BPFBTK, R-ENPLNB |
| s2 | 계좌 연동 관리 | R-BPFBTK, R-CCOEWW(F-KBBFRU 노출 설정) |
| s3 | 예산 설정 | R-JNPAKJ |
| s4 | 메인 홈 / 소비 피드백 | R-ENPLNB, R-JNPAKJ |
| s5 | 소비 분석 및 요약 | R-ZHBMMB |
| s6 | 거래 내역 관리 | R-CCOEWW |
| s7 | 절약 보상 및 보관함/상점 | R-VLQYRM |
| s8 | 프로필 및 설정 | (횡단) |
| **s9 (신규)** | **예산 기간 자동 전환** | R-JNPAKJ, R-ENPLNB, R-VLQYRM (시스템 배치) |

## 이번 업데이트로 추가된 노드

| 노드 | 타입 | 섹션 | 근거 |
|---|---|---|---|
| n67 계좌별 가계부 포함/제외 토글 설정 | action | s2 | F-KBBFRU: "계좌 관리 영역에 토글을 표시한다" |
| n68 제외로 설정? | branch | s2 | 토글이 "제외"일 때만 노출 방식 하위 옵션이 열림 |
| n69 노출 방식 선택(뱃지 표시/완전히 숨김) | action | s2 | F-KBBFRU 결정: 제외 계좌의 목록 노출 방식 선택 |
| n70 예산 기간 종료 | start | s9 | R-JNPAKJ 결정: 기간 종료가 배치의 시작점 |
| n71 새 예산 기간 자동 생성 | action | s9 | R-JNPAKJ 결정: 동일 목표로 자동 갱신 |
| n72 반려동물 상태 재계산 | action | s9 | R-VLQYRM 결정: 보상 판정 전에 상태부터 리셋 |
| n73 상점 화면 | page | s7 | F-HPWCNJ: 보관함 → 보관함+상점으로 확장 |
| n74 구매 가능 아이템 목록 | data | s7 | F-HPWCNJ dataSpec: 아이템(상점) 항목 |
| n75 아이템 선택 후 구매 시도 | action | s7 | F-HPWCNJ action: 크레딧으로 아이템 구매 |
| n76 크레딧 충분? | branch | s7 | F-HPWCNJ exceptions: 크레딧 부족 시 구매 차단 |
| n77 구매 완료(크레딧 차감) | action | s7 | F-HPWCNJ outcome: 구매 시 크레딧 즉시 차감 |
| n78 크레딧 부족 안내 | page | s7 | F-HPWCNJ exceptions: 부족 수량 안내 |

## 2026-09-15 업데이트로 추가·변경된 노드

| 노드 | 타입 | 섹션 | 근거 |
|---|---|---|---|
| n5 소셜 로그인 화면: 카카오/애플 | page | s1 | Q3: 이메일·비밀번호 로그인을 제공하지 않음 (기존 "로그인 화면"에서 변경) |
| n4 약관·개인정보 동의 화면 | page | s1 | Q6: 소셜 인증 성공 직후 이용약관·개인정보처리방침 동의 (기존 "회원가입 화면"을 대체) |
| n79 신규 가입? | branch | s1 | Q3: 기존 계정이면 동의 화면 없이 바로 메인 홈으로 진입 |
| n80 금융 데이터 조회 동의 화면 | page | s2 | Q6: 코드에프 제3자 제공·금융거래정보 조회 동의를 은행 인증 직전에 분리 |
| n81 회원 탈퇴 요청 | action | s8 | Q5: 스토어 심사 요건이자 개인정보 파기 의무 |
| n82 탈퇴 확인? | branch | s8 | Q5: 복구 불가를 안내하고 확인을 받음 |
| n83 계정·데이터 삭제 + connectedId 해지 | action | s8 | Q5: 즉시 전체 삭제, 코드에프 연결도 함께 해지 |
| n84 아이템 방 배치/해제 | action | s7 | Q8·Q16: 구매한 꾸미기 아이템을 반려동물의 방 4개 슬롯(벽지·바닥·집·장난감)에 배치, 결과가 메인 홈에 반영 |

## 전체 플로우 (Mermaid)

```mermaid
flowchart TD
  subgraph s1["섹션1: 인증 및 최초 설정"]
    n1([앱 최초 실행])
    n2([푸시 알림 진입])
    n3[온보딩/시작 화면]
    n4[약관·개인정보 동의 화면]
    n5[소셜 로그인 화면: 카카오/애플]
    n6{로그인 여부?}
    n79{신규 가입?}
    n7[최초 설정 화면]
    n8[캐릭터 선택 화면]
    n9(캐릭터 선택 저장)
    n10{캐릭터 선택 완료?}
  end

  subgraph s2["섹션2: 계좌 연동 관리"]
    n11[계좌 연결 화면]
    n12[(지원 은행 목록)]
    n13(은행 선택 후 연결 시작)
    n80[금융 데이터 조회 동의 화면]
    n14[은행 인증 화면]
    n15{인증 성공 여부?}
    n16[계좌 연결 완료 화면]
    n17[(연결 계좌 정보)]
    n18[인증 실패 안내 화면]
    n19[계좌 관리 화면]
    n20[(연결 계좌 목록)]
    n21(계좌 해제)
    n22(재인증 요청)
    n66[동기화 오류 안내 화면]
    n67(계좌별 가계부 포함/제외 토글 설정)
    n68{제외로 설정?}
    n69(노출 방식 선택: 뱃지 표시/완전히 숨김)
  end

  subgraph s3["섹션3: 예산 설정"]
    n23[예산 설정 화면]
    n24(예산 기간·목표 저장)
    n25[임계값 설정 화면]
    n26(임계값 수정 저장)
    n27{예산 설정 유효?}
    n28[예산 설정 오류 안내]
  end

  subgraph s4["섹션4: 메인 홈 / 소비 피드백"]
    n29[메인 홈 화면]
    n30[(반려동물 소비 피드백)]
    n31[(예산 사용률 요약)]
    n32{활성 예산 존재?}
    n33[상태 설명/범례 화면]
    n34{예산 사용률 100% 초과?}
    n35[예산 초과 경고 화면]
    n36(상태 설명 열기)
  end

  subgraph s5["섹션5: 소비 분석 및 요약"]
    n37[소비 요약 화면]
    n38[(핵심 소비 요약 수치)]
    n39[(주요 지출 카테고리)]
    n40(카테고리 분석으로 이동)
    n41(기간별 분석으로 이동)
    n42[카테고리별 분석 화면]
    n43[(카테고리별 지출 집계)]
    n44[기간별 분석 화면]
    n45[(기간별 지출 집계)]
  end

  subgraph s6["섹션6: 거래 내역 관리"]
    n46[거래 내역 화면]
    n47[(거래 내역 목록)]
    n48(거래 항목 선택)
    n49[거래 상세 화면]
    n50(분류 수정 시작)
    n51[거래 분류 수정 화면]
    n52(수정 내용 저장)
    n53{저장 성공 여부?}
    n54[수정 저장 실패 안내]
    n55(기간 선택)
  end

  subgraph s7["섹션7: 절약 보상 및 보관함/상점"]
    n56[절약 보상 획득 화면]
    n57[(칭찬 반응 및 크레딧 지급 정보)]
    n58[보상 보관함 화면]
    n59[(보유 아이템 목록)]
    n60(보유 아이템 선택)
    n61[보상 상세 화면]
    n62{절약 조건 충족?}
    n73[상점 화면]
    n74[(구매 가능 아이템 목록)]
    n75(아이템 선택 후 구매 시도)
    n76{크레딧 충분?}
    n77(구매 완료: 크레딧 차감)
    n78[크레딧 부족 안내]
    n84(아이템 방 배치/해제)
  end

  subgraph s8["섹션8: 프로필 및 설정"]
    n63[프로필/설정 화면]
    n64(캐릭터 변경)
    n65(예산 수정/종료)
    n81(회원 탈퇴 요청)
    n82{탈퇴 확인?}
    n83(계정·데이터 삭제 + connectedId 해지)
  end

  subgraph s9["섹션9: 예산 기간 자동 전환 (시스템 배치)"]
    n70([예산 기간 종료])
    n71(새 예산 기간 자동 생성)
    n72(반려동물 상태 재계산)
  end

  n1 --> n3
  n2 --> n29
  n3 --> n6
  n6 -- 아니오 --> n5
  n6 -- 예 --> n29
  n5 --> n79
  n79 -- 예 --> n4
  n79 -- 아니오 --> n29
  n4 --> n7
  n7 --> n8
  n8 --> n9
  n9 --> n10
  n10 -- 예 --> n11
  n10 -- 아니오 --> n8
  n11 --> n12
  n12 --> n13
  n13 --> n80
  n80 --> n14
  n14 --> n15
  n15 -- 예 --> n16
  n15 -- 아니오 --> n18
  n16 --> n17
  n17 --> n23
  n18 --> n11
  n19 --> n20
  n20 --> n21
  n20 --> n22
  n22 --> n14
  n21 --> n19
  n66 --> n19
  n20 --> n67
  n67 --> n68
  n68 -- 예 --> n69
  n69 --> n20
  n68 -- 아니오 --> n20
  n23 --> n24
  n24 --> n27
  n27 -- 예 --> n29
  n27 -- 아니오 --> n28
  n28 --> n23
  n23 --> n25
  n25 --> n26
  n26 --> n23
  n29 --> n30
  n29 --> n31
  n30 --> n32
  n32 -- 예 --> n34
  n32 -- 아니오 --> n23
  n34 -- 예 --> n35
  n34 -- 아니오 --> n29
  n29 --> n36
  n36 --> n33
  n35 --> n29
  n29 --> n37
  n37 --> n38
  n37 --> n39
  n39 --> n40
  n40 --> n42
  n38 --> n41
  n41 --> n44
  n42 --> n43
  n43 --> n46
  n44 --> n45
  n45 --> n46
  n29 --> n46
  n46 --> n47
  n47 --> n55
  n55 --> n47
  n47 --> n48
  n48 --> n49
  n49 --> n50
  n50 --> n51
  n51 --> n52
  n52 --> n53
  n53 -- 예 --> n49
  n53 -- 아니오 --> n54
  n54 --> n51
  n29 --> n62
  n62 -- 예 --> n56
  n62 -- 아니오 --> n29
  n56 --> n57
  n57 --> n58
  n58 --> n59
  n59 --> n60
  n59 --> n84
  n84 --> n29
  n60 --> n61
  n29 --> n63
  n63 --> n64
  n64 --> n8
  n63 --> n65
  n65 --> n23
  n63 --> n81
  n81 --> n82
  n82 -- 예 --> n83
  n82 -- 아니오 --> n63
  n83 --> n1
  n63 --> n19
  n63 --> n58
  n46 --> n66
  n29 --> n19
  n58 --> n73
  n73 --> n74
  n74 --> n75
  n75 --> n76
  n76 -- 예 --> n77
  n77 --> n58
  n76 -- 아니오 --> n78
  n78 --> n73
  n70 --> n71
  n71 --> n72
  n72 --> n29
```

## 주요 분기 요약

| 분기 노드 | 조건 | 예(Yes) | 아니오(No) |
|---|---|---|---|
| n6: 로그인 여부? | 기존 세션 존재 | 메인 홈(n29)으로 바로 진입 | 소셜 로그인 화면(n5)으로 이동 |
| **n79: 신규 가입?** (신규) | 해당 소셜 계정의 기존 계정 없음 | 약관·개인정보 동의 화면(n4) → 최초 설정(n7) | 메인 홈(n29)으로 바로 진입 |
| n10: 캐릭터 선택 완료? | 캐릭터 저장됨 | 계좌 연결 화면(n11) | 캐릭터 선택 화면(n8) 유지 |
| n15: 인증 성공 여부? | 코드에프 인증 통과 | 계좌 연결 완료 화면(n16) | 인증 실패 안내(n18) → 재시도 |
| n27: 예산 설정 유효? | 구간 검증 통과 | 메인 홈(n29) | 예산 설정 오류 안내(n28) |
| n32: 활성 예산 존재? | 예산 설정 완료됨 | 사용률 검사(n34) | 예산 설정 화면(n23)으로 유도 |
| n34: 예산 사용률 100% 초과? | 초과 여부 | 예산 초과 경고(n35) | 메인 홈 유지 |
| n53: 저장 성공 여부? | 분류 수정 저장 결과 | 거래 상세 화면(n49) | 저장 실패 안내(n54) |
| n62: 절약 조건 충족? | 새 기간 진입 시 배치 판정 결과 확인 | 절약 보상 획득(n56) | 메인 홈(n29) |
| **n68: 제외로 설정?** (신규) | 계좌를 가계부 집계에서 제외로 토글 | 노출 방식 선택(n69) | 계좌 목록으로 복귀 |
| **n76: 크레딧 충분?** (신규) | 보유 크레딧 ≥ 아이템 가격 | 구매 완료(n77) | 크레딧 부족 안내(n78) |
| **n82: 탈퇴 확인?** (신규) | 사용자가 복구 불가 안내를 확인하고 동의 | 계정·데이터 삭제(n83) → 앱 최초 실행(n1) | 프로필/설정(n63)으로 복귀 |

## 가입 및 동의 흐름 (신규, s1·s2)

Q3·Q6 결정을 반영해 동의를 두 시점으로 나눴다.

```
n3 온보딩/시작 화면
  → n6 로그인 여부?
      예 → n29 메인 홈
      아니오 → n5 소셜 로그인 화면(카카오/애플)
          → n79 신규 가입?
              예 → n4 약관·개인정보 동의 화면 → n7 최초 설정 화면
              아니오 → n29 메인 홈
...
n13 은행 선택 후 연결 시작
  → n80 금융 데이터 조회 동의 화면 (코드에프 제3자 제공 + 금융거래정보 조회)
  → n14 은행 인증 화면
```

이용약관·개인정보처리방침은 가입 시점에, 금융 데이터 관련 동의는 계좌 연결을 실제로 시작하는 시점에 받는다. 동의를 받는 목적과 그 목적이 발생하는 시점이 일치해 법적으로 안전하고, 계좌 연결을 아직 시작하지 않은 가입 단계의 장벽도 낮아진다. 두 동의 모두 사용자 식별자·동의 종류·동의 버전·동의 시각으로 기록한다.

## 회원 탈퇴 흐름 (신규, s8)

```
n63 프로필/설정 화면
  → n81 회원 탈퇴 요청
  → n82 탈퇴 확인?
      예 → n83 계정·데이터 삭제 + connectedId 해지 → n1 앱 최초 실행
      아니오 → n63으로 복귀
```

유예 기간이나 복구 경로가 없다. n83에서 거래·연결 계좌·예산 기간·상태 구간·캐릭터 선택·크레딧·보상 기록·보유 아이템·동의 기록·디바이스 토큰을 모두 삭제하고 모든 세션을 무효화한다. 코드에프 연결 해지에 실패해도 서비스 측 삭제는 진행하되 실패한 해지 건은 재시도 대상으로 기록한다.

## 예산 기간 자동 전환 흐름 (신규, s9)

R-JNPAKJ·R-VLQYRM 검토에서 확정한 이벤트 순서를 그대로 다이어그램에 옮겼다.

```
n70 예산 기간 종료 (시스템 트리거, 사용자 조작 없음)
  → n71 새 예산 기간 자동 생성 (동일 목표 금액·동일 상태 구간으로)
  → n72 반려동물 상태 재계산 (새 기간 기준으로 리셋)
  → n29 메인 홈 화면 (사용자가 다음에 앱을 열면 이미 리셋된 상태)
  → n62 절약 조건 충족? (새 기간 진입 시 이 배치 결과를 확인)
```

**배치 실행 시점**: Q2 결정에 따라 매일 KST 00:05에 실행하며, 종료 시각이 지난 예산 기간을 모두 찾아 처리한다. 예산 기간은 매월 1일 00:00(KST)에 시작해 말일 24:00에 끝나는 월간 고정 기간이다(Q1).

n1/n2처럼 시작 노드(`([Label])`)로 표기했지만, 사용자가 직접 트리거하는 게 아니라 **예산 기간 종료 시각에 시스템이 자동으로 실행**하는 배치 작업이라는 점이 다르다. Claude Code로 구현할 때 이 세 노드(n70→n71→n72)는 하나의 원자적 배치 잡으로 묶어서, 중간에 실패해도 "새 기간은 생겼는데 상태 재계산은 안 된" 상태가 남지 않게 해야 한다(`04-review-log.md`에 동일 주의사항 기록됨).

## 계좌 노출 설정 흐름 (신규, s2)

```
n20 연결 계좌 목록
  → n67 계좌별 가계부 포함/제외 토글 설정
  → n68 제외로 설정?
      예 → n69 노출 방식 선택(뱃지 표시 / 완전히 숨김) → n20으로 복귀
      아니오(포함 유지) → n20으로 복귀
```

n69에서 "완전히 숨김"을 선택하면 F-RNECMQ·F-TKSQSR·F-IWBASY 세 화면 모두에서 해당 계좌 거래가 조회조차 안 되고, "뱃지 표시"를 선택하면 목록엔 남되 집계에서만 빠진다 — 이 분기가 R-CCOEWW·R-ZHBMMB 두 요구사항의 화면 동작을 동시에 결정한다.

## 상점 흐름 (신규, s7)

```
n58 보상 보관함 화면
  → n73 상점 화면
  → n74 구매 가능 아이템 목록
  → n75 아이템 선택 후 구매 시도
  → n76 크레딧 충분?
      예 → n77 구매 완료(크레딧 차감) → n58로 복귀 (보관함에 새 아이템 반영)
      아니오 → n78 크레딧 부족 안내 → n73로 복귀

n58 보상 보관함 화면
  → n59 보유 아이템 목록
  → n84 아이템 방 배치/해제
  → n29 메인 홈 (배치 결과가 방 표시에 반영)
```

n84는 Q8 결정으로 추가됐고 Q16에서 방식이 바뀌었다. 아이템은 캐릭터에 입히는 것이 아니라 반려동물의 방을 꾸민다. 슬롯은 벽지·바닥·집·장난감 4개이며 각 1개씩 배치하고, 렌더링 순서는 벽지 → 바닥 → 집 → 캐릭터 → 장난감이다. 상태 6단계가 바뀌어도 배치는 유지된다. 이 노드가 없으면 구매(n77)에서 보상 루프가 끊겨 크레딧을 모을 이유가 사라진다.

## 허브 구조

`n29 메인 홈 화면`이 전체 플로우의 중심 허브다. 소비 피드백(s4), 소비 요약(s5), 거래 내역(s6), 절약 보상(s7), 프로필/설정(s8), 계좌 관리(s2)로 모두 직접 연결되고, 이번 업데이트로 **s9(예산 기간 자동 전환)도 n29로 직접 연결**되는 두 번째 진입 경로가 생겼다. `n58 보상 보관함 화면`은 이번 업데이트로 상점(n73)까지 아우르는 s7 내부의 보조 허브 역할을 겸하게 됐다. Claude Code로 라우팅/네비게이션 구조를 짤 때 `HomeScreen`을 중심 허브 컴포넌트로, s9의 세 노드는 화면이 아니라 백그라운드 배치 잡으로 구현하는 근거가 된다.
