...
### 비정규화한 `user_id` (2026-09-22)

세 테이블이 부모를 통해 이미 알 수 있는 `user_id`를 따로 갖는다.

| 테이블 | 부모 | 근거 |
|---|---|---|
| `transactions` | `accounts` | 집계가 전부 "사용자 + 기간" 기준이라 `accounts` 조인이 모든 집계 쿼리에 붙는다. 사용자당 거래가 수천 건 이상 쌓여 손해가 크다(3.4절) |
| `reward_grants` | `budget_periods` | 유니크 `(user_id, budget_period_id, condition_type)`의 선행 열이자 "이 사용자의 보상 이력" 조회 경로다 |
| `push_logs` | `budget_periods` | 탈퇴 시 CASCADE 경로이자 사용자별 발송 이력 조회 경로다 |

`transactions`의 진짜 값은 조인 제거보다 **정렬**이다. 거래 목록은 전체 계좌를 통합해 최신순으로 보여주는데, `user_id`가 있으면 인덱스 `(user_id, transacted_at DESC)` 하나를 위에서부터 읽으면 된다. 없으면 계좌 수만큼 인덱스를 스캔해 병합해야 하고 페이지네이션이 깊어질수록 나빠진다.

**어긋남은 DB가 막는다 (2026-09-23).** 부모에 `UNIQUE (id, user_id)`를 걸고 자식이 그 쌍을 통째로 참조하는 복합 FK를 건다.
...
**비용을 재고 결정했다.** 5만 건 INSERT(90일치 최초 수집 상당)에서 단일 FK 242.9ms ↔ 복합 FK 248.1ms로 2.4% 차이였다. 자식에 추가되는 열은 없다 — 두 열 모두 이미 있다. 부모의 유니크 인덱스는 PK와 중복처럼 보이지만 복합 FK의 참조 대상에는 유니크 제약이 필요하고, 두 테이블 모두 사용자당 행 수가 적어 비용이 작다.

**그 전에는 코드 규율로만 막고 있었다** — "이 세 테이블에 행을 만들 때는 반드시 부모를 조회한 경로에서 `user_id`를 가져온다". 규율이 약한 이유는 어긋나도 **예외도 경고도 나지 않기** 때문이다. 사용자 A의 예산 사용률에 B의 거래가 섞여 들어가고 금액이 이상하다는 신고를 받기 전까지 알 수 없다. 금융 데이터라 더 나쁘다.

실제로 T-031에서 이 틈이 드러났다. `push_logs`의 유니크가 `(budget_period_id, threshold_type)` 2열인데, `user_id`를 더한 3열로 바꿔도 동작 단언 셋이 전부 통과했다. 같은 기간을 다른 `user_id`로 기록하는 행을 만들 수 있었기 때문이다. 상세는 트러블슈팅 21번.

**규율은 여전히 필요하다.** DB가 거부하는 것은 저장 시점이므로, 어긋난 값을 넘기면 INSERT가 실패한다. 막아 주는 것이지 알아서 고쳐 주는 것이 아니다.

### FK 자식 컬럼에는 인덱스를 건다 (2026-09-22)

**PostgreSQL은 FK의 참조하는 쪽(자식) 컬럼에 인덱스를 자동으로 만들지 않는다.** PK 쪽에만 인덱스가 생긴다. 부모 행을 지울 때 DB는 "이 행을 가리키는 자식이 있나"를 확인해야 하는데, 자식 컬럼에 인덱스가 없으면 **자식 테이블 전체를 훑는다.**

**반복 횟수는 `ON DELETE` 규칙에 따라 다르다 (2026-09-23 정정).**

| 규칙 | 스캔 횟수 | 예 |
|---|---|---|
| `NO ACTION` · `RESTRICT` | **삭제되는 자식 행마다** | `transactions.linked_refund_transaction_id` |
| `CASCADE` · `SET NULL` | **삭제되는 부모 행마다** | `sync_attempts.user_id`, `reward_grants.budget_period_id` |

처음에는 둘 다 "자식 행마다"로 적었는데 **CASCADE에는 틀렸다.** 실측으로 확인했다 — 삭제되는 자식 수를 같게 두고 부모 수만 바꿨다.
...
자식 수는 거의 같은데 70배 차이다. CASCADE는 부모 한 행마다 `DELETE FROM 자식 WHERE fk = ?`를 한 번씩 돌린다.

아래 측정은 `NO ACTION`인 자기참조 FK의 것이라 "자식 행마다"가 맞다.
...
**CASCADE 쪽 셋도 인덱스는 필요하다.** 반복 횟수가 적을 뿐 한 번의 스캔이 큰 테이블 전체다. 특히 `reward_grants.budget_period_id`는 탈퇴 시 예산 기간 수(연 12개)만큼 반복되어 셋 중 가장 나쁘다.
