...
**대가는 세 테이블이 같다. `user_id`와 부모의 사용자가 어긋날 수 있고 DB가 막아 주지 않는다.** 복합 FK로 강제하려면 부모에 `UNIQUE (id, user_id)`를 추가해야 하는데, 명세에 없는 제약을 더하는 일이라 하지 않았다.

**규율로 지킨다: 이 세 테이블에 행을 만들 때는 반드시 부모를 조회한 경로에서 `user_id`를 가져온다.** 호출자가 넘긴 값을 그대로 쓰지 않는다.
