...
| 23 | FK 자식 컬럼에 인덱스가 없어 부모 삭제가 자식 테이블 전체를 훑는다 | **조용한 실패** / 설계 문서 누락 | — |

**가장 많이 나온 유형은 "조용한 실패"다.** 빌드도 기동도 성공하는데 기능만 비어 있는 경우가 11건이었다. 이 유형이 위험한 이유는 아래 마지막 절에 정리했다.
...
23건 중 11건(3·4·9·10·11·17·18·19·20·22·23번)이 **빌드도 기동도 성공하는데 기능만 비어 있는** 유형이었다.
...
PostgreSQL은 FK의 **참조하는 쪽(자식) 컬럼에 인덱스를 자동으로 만들지 않는다.** PK 쪽에만 생긴다. 부모 행을 지울 때 "이 행을 가리키는 자식이 있나"를 확인해야 하는데, 인덱스가 없으면 자식 테이블 전체를 훑고 이것이 삭제되는 행마다 반복된다.
