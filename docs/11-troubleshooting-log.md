...
| 21 | 동작 단언 셋으로는 유니크의 열 구성이 고정되지 않는다 — `user_id`를 더해도 셋이 그대로 통과한다 | 검증 방법 / 테스트 설계 | — |
...
처음에는 동작 단언이 부실한 줄 알았다. 아니었다. **3열 유니크는 2열 유니크보다 느슨하다.**
2열이 거부하는 조합의 집합이 3열이 거부하는 집합을 포함한다. 두 유니크의 동작이 갈리려면
`budget_period_id`가 같은데 `user_id`가 다른 두 행이 필요한데, `push_logs.budget_period_id`는
`budget_periods`를 가리키고 그 행에 `user_id`가 이미 박혀 있어 **그런 두 행을 만들 수 없다.**
명세가 "`budget_period_id`가 이미 사용자를 함의한다"고 적은 것이 바로 그 뜻이고, 같은 이유로
두 유니크는 이 스키마에서 **동작이 구별되지 않는다.**

같은 논리가 T-041의 `reward_grants`에는 적용되지 않는다. 그쪽은 `user_id`를 빼면
`UNIQUE (budget_period_id, condition_type)`가 되어 여전히 동작이 같지만, 반대로 이쪽에서
`budget_period_id`를 빼면(M3 변이) 동작이 크게 달라진다 — 그 변이는 18건 중 8건을 깼다.
**열을 더하는 변이와 빼는 변이는 대칭이 아니다.**

**원인**

동작 단언은 "제약이 무엇을 막고 무엇을 허용하는가"만 본다. 스키마에 구조적으로 도달할 수 없는
상태가 있으면 **서로 다른 두 제약이 같은 동작을 낸다.** 완료 기준이 동작 셋만 요구했다면
이 축은 테스트로 고정되지 않은 채로 초록불이었을 것이다.

**해결**

코드 변경은 없었다. 이미 있던 구조 단언이 잡았다.
...
`assertIndex`는 `indexdef`의 꼬리를 `endsWith`로 보므로 열 구성과 순서가 그대로 걸린다.
동작으로 구별되지 않는 변이를 구조로 잡은 셈이다.

**검증**

변이 14종을 하나씩 넣고 `PushSchemaTest`만 돌린 뒤 매번 원복했다. 열 구성 관련 넷만 옮긴다.

```
### M1 push_logs 유니크 통째 제거        → 18 tests, 2 failed
    같은 기간에 같은 임계값을 두 번 보낼 수 없다 / 설계한 인덱스가 실제로 만들어져 있다
...
**동작 단언과 구조 단언은 서로를 대체하지 않는다.** 17·18·19번은 구조 단언이 없어서 새어
나간 경우였고, 20번은 축의 경로가 좁아서였다. 이번은 그 반대 방향이다 — 동작 단언이 셋이나
있었는데도 스키마가 그 셋을 구별하지 못했다. 제약을 **느슨하게** 만드는 변이는 종종 동작으로
드러나지 않으므로, 제약의 열 구성 자체를 `pg_indexes`로 못 박아 두는 단언이 따로 있어야 한다.

**"두 테이블이 비슷하니 제약도 맞추자"가 위험한 이유가 여기 있다.** `reward_grants`에 맞춰
`user_id`를 더해도 이 스키마에서는 아무 테스트도 깨지지 않는다. 명세가 2열로 정한 근거
(`budget_period_id`가 사용자를 함의한다)는 그 자체가 **두 제약을 동작으로 구별할 수 없게 만드는
바로 그 사실**이라, 근거를 아는 사람만 차이를 지킬 수 있다. 그래서 마이그레이션 주석과 테스트
주석 양쪽에 "`reward_grants`와 달리 2열"을 남겼다.
...
- 푸시 스키마: 변이 14종(유니크 5, CHECK 1, FK 3, `NOT NULL` 1, 인덱스 1, 엔티티 3)을 하나씩 넣고 원복해, 각각 **어느 테스트가** 깨지는지까지 확인. 유니크에 열을 *더하는* 변이만 동작 단언을 통과하고 구조 단언에 걸리는 것이 여기서 드러났다(21번)
