...
- 최종 갱신: 2026-09-15 (MVP 재점검 Q1~Q14 결정 반영)
- **전제**: MVP는 Flutter 모바일 앱 단독 출시다(Q14). 아래 프론트 항목은 모두 모바일 기준이며 웹 화면 작업은 제거했다. 각 스프린트의 테스트 항목은 완료 판정 기준이다.
...
- [ ] User 엔티티 (사용자식별자, 공급자종류 kakao/apple, 공급자사용자식별자, 가입시각, 마지막로그인시각) — **비밀번호 필드 없음**, (공급자종류, 공급자사용자식별자) 유니크
- [ ] F-QJXRMD: 카카오·애플 소셜 로그인 API (Spring Security + JWT)
- [ ] 토큰 정책 — 액세스 30분 / 리프레시 14일, 리프레시 회전, 폐기 토큰 재사용 감지 시 전체 세션 무효화. 폐기 상태는 Upstash에 TTL 14일 저장
- [ ] 약관·개인정보처리방침 동의 API, 동의 기록 테이블 (사용자, 동의종류, 동의버전, 동의시각)
- [ ] F-ZPNVKT: 회원 탈퇴 API — 전체 데이터 즉시 삭제 + 코드에프 connectedId 해지(Sprint 1 이후 연동), 세션 무효화
- [ ] 가입 이벤트 로깅 — 가입시각 t=0 기준 24시간·7일 코호트 윈도우를 적재 단계에서 분리
- [ ] 헬스체크 엔드포인트
- [ ] 테스트: 소셜 로그인 성공/실패, 신규 가입 시 동의 필수, 리프레시 회전, 폐기 토큰 재사용 시 전체 세션 무효화, 탈퇴 후 데이터 전멸 확인
- [ ] 프론트(모바일): 온보딩·소셜 로그인·약관 동의 화면

**완료 기준**: 카카오 또는 애플로 로그인해서 Cloud Run 배포된 API를 호출하면 응답이 오고, 탈퇴하면 해당 사용자 데이터가 남지 않는다. 위 테스트가 모두 통과한다.

**선행 조건(코드 작업 아님)**: 카카오 개발자 애플리케이션 등록·동의 항목 설정, 이용약관·개인정보처리방침 문안 준비. 둘 다 이 스프린트 착수 전에 필요하다.

**애플 로그인은 이 스프린트의 선행 조건이 아니다.** Apple Developer Program 가입(연 $99)은 아래 두 작업에만 필요하며, iOS 출시를 착수하는 시점으로 미룬다.

| 작업 | 멤버십 필요 여부 |
|---|---|
| 백엔드의 애플 identity token 검증 코드 작성 | 불필요 — 검증용 공개키(`appleid.apple.com/auth/keys`)는 공개다 |
| 유효한 애플 토큰을 발급받아 종단 테스트 | 필요 |
| iOS 빌드·TestFlight·App Store 심사 제출 | 필요 |

App Store 심사 지침 4.8(Login Services)에 따라, iOS 앱이 카카오 로그인을 제공하면 데이터 수집을 이름·이메일로 제한하고 이메일 가리기를 지원하는 로그인 옵션을 함께 제공해야 한다. 카카오 로그인은 이 조건을 충족하지 않으므로 **iOS 출시 시점에는 Sign in with Apple 병행이 사실상 의무**다. 이 스프린트에서는 카카오 로그인만 완성하고, 애플 경로는 iOS 착수 시 추가한다.
...
- [ ] 금융 데이터 조회 동의 API — 코드에프 제3자 제공 + 금융거래정보 조회. **은행 인증 시작 직전에 받고, 가입 시 동의와 분리해 기록**(Q6)
- [ ] 계좌별 "재인증 필요" 상태 필드와 사용자 유도 흐름 — 2-way 추가인증 은행의 무인 동기화 불가 케이스 대비(C-3)
- [ ] 탈퇴 시 connectedId 해지 연동 (Sprint 0의 탈퇴 API에 연결), 해지 실패 건 재시도 기록
- [ ] 예외처리: 인증실패·대행사 장애 시 연결정보 미저장
- [ ] 테스트: 복수 계좌 배열 저장, 동의 없이 연결 시작 차단, 계좌 해제 후 동기화 중단
- [ ] 프론트(모바일): 은행 목록 화면, 금융 동의 화면, 인증 화면, 연결완료 화면
...
- [ ] 카테고리 10개 시드 데이터 (식비 / 카페·간식 / 교통 / 쇼핑 / 의료·건강 / 문화·여가 / 주거·통신 / 금융·보험 / 경조사비 / 미분류) — 사용자 추가·삭제 불가, 관리자 화면 없이 마이그레이션으로 관리(Q7·Q12)
- [ ] 가맹점명 키워드 룰 테이블 (JSONB+GIN), 자동 카테고리 분류 — 룰도 시드 데이터로 관리
- [ ] Transaction에 **최초 분류 출처 필드**(자동 매칭 / 미분류) — 사용자 수정 시에도 갱신 금지, KPI 계측용(Q11)
...
- [ ] 테스트: 중복거래 미저장, 이체 자동매칭 3조건 AND, 환불 순액 계산, 미매칭 거래의 미분류 처리, 분류 출처 필드가 수정 후에도 불변

### F-RNECMQ
- [ ] 거래목록 조회 API (기간필터, 최신순)
- [ ] 가계부 제외 계좌 뱃지/완전숨김 표시 로직
- [ ] 거래 상세 API (원거래-환불 연결관계)
- [ ] 빈 상태 안내에 "연결한 계좌의 거래만 집계된다" 문구 — 수동 거래 입력은 MVP 제외(Q10)
- [ ] 프론트(모바일): 거래목록·상세 화면, 빈상태 처리
...
### F-FZUVLV
- [ ] BudgetPeriod/StatusThreshold 엔티티 — BudgetPeriod는 기간시작일·기간종료일·목표금액·기간시작시점 목표금액 스냅샷 포함. **기간은 매월 1일 00:00(KST)~말일 24:00 월간 고정, 사용자가 시작일 변경 불가**(Q1)
- [ ] 기본 임계값 6단계 프리필(휴식0~40%~예산초과100%~)
- [ ] 구간 검증 (0% 고정, 오름차순, 중복·공백 검증)
- [ ] 예산 기간/목표 설정 API
- [ ] 사용률 실시간 계산 함수 (누적지출÷목표금액)
- [ ] 오른쪽닫힘 경계값 판정 함수 (`(시작,끝]`, 0%는 예외)
- [ ] 모든 예산 시각 계산의 기준 타임존 KST 고정
- [ ] 테스트: 구간 검증(0% 고정·오름차순·중복·공백), 경계값 판정(40%는 휴식, 0%는 휴식), 사용률 계산에서 이체·제외계좌 제외
- [ ] 프론트(모바일): 예산설정 화면, 임계값 표 편집 UI

### F-GGIDHG
- [ ] Character 엔티티
- [ ] 캐릭터 선택/변경 API
- [ ] Lottie 에셋 12개 Cloud Storage 업로드
- [ ] 프론트(모바일): 캐릭터 선택 화면, lottie 렌더링
...
- [ ] 노출방식(뱃지/숨김) 하위옵션 API
- [ ] 수정이력 기록
- [ ] 테스트: 수정 결과가 예산 사용률·카테고리 집계에 즉시 반영, 이체 연결해제 후 원래 유형으로 재계산
...
- [ ] 상태설명/범례 API
- [ ] 디바이스 토큰 테이블(사용자, FCM토큰, 플랫폼, 갱신시각) + 토큰 등록·갱신 API
- [ ] 90%/100% 진입감지, 기간당1회 발송기록(Upstash 키: `budget_period_id:threshold`)
- [ ] **푸시 재발송 금지** — 환불·목표상향으로 사용률이 내려갔다가 재진입해도 같은 기간에는 재발송 안 함(Q13)
- [ ] FCM 발송 (`firebase-admin`, ADC 인증 확인) — **모바일 전용, 웹 푸시 없음**(Q9)
- [ ] 푸시 발송 로그 테이블(사용자, 예산기간, 임계값종류, 발송시각, 열람시각) + 열람 보고 API — KPI "예산 초과 경고 확인율" 계측(Q11)
- [ ] 메인 홈에 장착 아이템 겹쳐 표시 (Sprint 6의 F-HPWCNJ와 연결, 필드만 선반영)
- [ ] 테스트: 사용률별 6단계 매핑, 90%/100% 각 1회만 발송, 재진입 시 미발송, 예산 계산과 상태 계산이 같은 집계 함수를 쓰는지
- [ ] 프론트(모바일): 메인홈, 상태설명, 예산초과 경고 화면, 알림 권한 요청 흐름, 알림 탭 이벤트 전송
...
- [ ] 테스트: 카테고리별 합계 = 총지출, 기간별 합계 = 총지출, 제외 계좌 거래가 집계에서 빠지는지
- [ ] 프론트(모바일): 소비요약 화면
...
- [ ] 배치 스케줄 — **매일 KST 00:05 실행, 종료 시각이 지난 예산 기간을 전부 스캔**(Q2). ShedLock 분산락 재사용
...
- [ ] 상점 아이템 시드 데이터 (100~500 크레딧 등급별) — 관리자 화면 없이 마이그레이션 관리(Q12)
- [ ] **아이템 장착/해제 API** — 보유 아이템 기록에 장착여부 필드, 장착 결과가 메인 홈 캐릭터 표시에 반영(Q8)
- [ ] 테스트: 배치 중간 실패 시 롤백(새 기간만 생기고 재계산 안 된 상태가 남지 않음), 크레딧 동시 차감 정합성, 잔액 부족 시 구매 차단, 미보유 아이템 장착 차단, 첫 기간 절약 판정 생략
- [ ] 프론트(모바일): 보상획득 화면, 보관함/상점 화면, 장착 UI

**선행 조건(코드 작업 아님)**: 꾸미기 아이템 에셋 설계. 상태 6단계 애니메이션 위에 겹치는 별도 레이어로 제작해야 하며, 어느 상태 포즈에서도 어색하지 않도록 위치·크기 기준을 Lottie 에셋 제작 단계에서 함께 정해야 한다.
