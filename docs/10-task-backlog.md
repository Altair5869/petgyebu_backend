...
| [ ] | T-003a | 액세스 토큰 발급·검증 | `feature/F-QJXRMD-token-access` | 액세스 30분으로 발급된다. 토큰 없음·서명 불일치·만료가 각각 401이고, 유효 토큰은 컨트롤러까지 `userId`를 전달한다 | T-001 | — |
...
| 차단 없음 | 44 |
| 외부 조건에 막힌 것 | 10 |
| 완료 | 10 (T-047, T-001, T-052, T-006, T-013, T-014, T-023, T-031, T-040, T-041) |
...
**차단 없는 44개 중 34개는 앞선 Task에 의존해 순서를 기다리는 것뿐이다.** 외부 조건과 무관하므로 앞 단계가 끝나는 대로 이어서 진행할 수 있다.
