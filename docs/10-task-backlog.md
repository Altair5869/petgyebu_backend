...
| **B-REDIS** | Upstash Redis 미프로비저닝 | Upstash 계정 생성 후 인스턴스 생성 | T-003b, T-009, T-020, T-021, T-050 |
...
| [x] | T-003a | 액세스 토큰 발급·검증 | `feature/F-QJXRMD-token-access` | ✅ 완료. QA PASS 8 / FIX 3 / REDO 0 / 미검증 1(탈퇴 후 토큰 30분 잔존 → T-003b로 이월) | T-001 | #27 |
| [ ] | T-003b | 리프레시 토큰 발급·회전 | `feature/F-QJXRMD-token-refresh` | 리프레시 14일로 발급된다. 리프레시 사용 시 이전 토큰이 폐기되고, 폐기 토큰 재사용이 감지되면 전 세션이 무효화된다. **탈퇴한 사용자의 기존 액세스 토큰도 거부된다** | T-003a | **B-REDIS** |
| [ ] | T-004 | 약관·개인정보 동의 API | `feature/F-QJXRMD-consent` | 필수 동의 없이는 가입이 완료되지 않는다. 동의 기록에 종류·버전·시각이 남는다 | T-001 | **B-TERMS** |
| [ ] | T-005 | 회원 탈퇴 API | `feature/F-ZPNVKT-withdrawal` | 탈퇴 후 해당 사용자의 모든 행이 사라진다(`ON DELETE CASCADE` 확인). 전 세션이 무효화된다 | T-001, T-003b | — |

**T-003을 T-003a/T-003b로 쪼갰다** (2026-09-23): 폐기 상태 저장을 Upstash에 두기로 했으므로(`09-db-design.md` 2.2절) 리프레시 쪽은 실제 Redis 없이 종단 검증이 안 된다. 반면 **T-025(예산 설정 API)를 막는 것은 액세스 토큰 검증 경로뿐**이라, 거기까지만 T-003a로 떼어 먼저 했다. 액세스 토큰은 Redis를 쓰지 않아 차단이 없다.

**액세스 토큰에는 무효화 경로가 없다** (2026-09-23, T-003a QA): 필터가 DB를 조회하지 않아 탈퇴·삭제된 사용자의 액세스 토큰이 최대 30분 살아 있다. T-003a 범위 위반은 아니지만, `02-requirements-features.md` **F-ZPNVKT**가 요구하는 "모든 세션 무효화"는 리프레시 폐기(T-003b)만으로 완성되지 않는다. T-003b 완료 기준에 넣었다. 30분 잔존을 수용하기로 바꾼다면 그 근거를 여기에 남긴다.

T-003a는 `TokenService`(서비스 계층)까지만이고 **발급용 HTTP 엔드포인트가 없다** — 카카오 인가 코드(T-002)가 없어 진입점이 없기 때문이다. T-002가 붙을 때 그 컨트롤러가 이 서비스를 호출한다. 테스트는 `TokenService`로 진짜 토큰을 발급해 헤더에 실어 보내므로 모의 인증 없이 종단이 돈다.
...
| 차단 없음 | 44 |
| 외부 조건에 막힌 것 | 10 |
| 완료 | 11 (T-047, T-001, T-052, T-006, T-013, T-014, T-023, T-031, T-040, T-041, T-003a) |
...
| B-TERMS | T-004 |
| B-CODEF | T-009, T-015 |
| B-REDIS | T-003b, T-009, T-020, T-021, T-050 |
| B-GCP | T-048, T-049 |

**차단 없는 44개 중 33개는 앞선 Task에 의존해 순서를 기다리는 것뿐이다.** 외부 조건과 무관하므로 앞 단계가 끝나는 대로 이어서 진행할 수 있다.
...
| **비정규화 소유자 정합성** | 3 | 복합 FK + 어긋남 거부 테스트 (2026-09-23 추가) |
| **마이그레이션 불변성** | — | `check-migration-order.sh`가 병합된 파일의 수정·삭제를 막는다 (2026-09-23 추가) |
...
**비정규화한 열은 DB가 강제하게 한다 (2026-09-23 추가)**

부모를 통해 알 수 있는 값을 자식에 복사해 두면 어긋날 수 있다. 어긋나도 **예외도 경고도 나지 않는다** — 집계에 남의 데이터가 섞이고 금액이 이상하다는 신고를 받기 전까지 모른다.

**코드 규율("항상 부모를 조회한 경로에서 가져온다")은 약한 방어선이다.** 앞으로 그 테이블에 쓰는 모든 코드가 규칙을 알아야 하고, 어기면 조용히 틀린다. 부모에 `UNIQUE (id, 복사한열)`을 걸고 자식이 그 쌍을 참조하는 **복합 FK**로 바꾸면 물리적으로 불가능해진다.

비용을 재고 결정했다 — 5만 건 INSERT에서 2.4%였고 자식에 추가되는 열은 없다. 상세는 `docs/09-db-design.md` 0장.

**병합된 마이그레이션은 고치지 않는다 (2026-09-23 추가)**

Flyway가 적용한 마이그레이션의 checksum을 `flyway_schema_history`에 남긴다. 파일이 나중에 바뀌면 기동 시 `FlywayValidateException`이 나고 애플리케이션이 아예 뜨지 않는다. **주석 한 줄만 고쳐도 같다.**

이 실패는 빈 DB에서 재현되지 않는다. 실측:
...
`scripts/check-migration-order.sh`가 `--diff-filter=A`로 **추가된** 파일만 보고 있어 수정·삭제는 아무도 잡지 않았다. 같은 스크립트에 불변성 검사를 더했다. 트러블슈팅 16번(순서)과 같은 계열이다 — 그때는 순서를 막고 수정은 열어뒀다.

바꿀 내용이 있으면 **새 마이그레이션을 추가한다.**
...
**이번 Task가 만든 엔티티는 최소 한 번 리포지토리로 저장하고 되읽어 넘긴 값과 같은지 확인한다.**
...
**축 7 보강 (2026-09-22, T-031 QA에서)**: "되읽는다"까지만으로는 부족하다. T-031에서 엔티티 경로 단언이 `fcm_token`·`platform`·FK·열거형은 값을 비교했는데 **시각 셋만 `isNotNull()`이었다.** 생성자에서 `sentAt`에 `.plusDays(1)`을 붙이거나 `updatedAt`에 `.plusYears(1)`을 붙여도 18건이 전부 통과했다. **넘긴 값과 같은지 비교하라.**

시각은 정밀도에 주의한다. JDK 시계는 나노, PostgreSQL `timestamptz`는 마이크로라 그대로 비교하면 값이 맞아도 어긋날 수 있다. 밀리초로 잘라 비교한다(`ChronoUnit.MILLIS`). 검출 대상이 날·해 단위로 밀린 시각이므로 이 정밀도로 충분하다.

**완료 기준이 열거한 방향이 전부라고 가정하지 마라 (2026-09-22, T-031)**

T-031의 완료 기준 2는 유니크를 동작 **세 방향**으로 확인하라고 적었다. 셋을 다 넣었는데도 유니크에 `user_id`를 더하는 변이에서 **동작 단언 셋이 전부 통과하고 구조 단언 하나만 걸렸다.**
